...
if [ "$MODE" = fuzz ]; then
  ...
  # Seed the container target BEFORE instrumenting, and the order is not cosmetic.
  #
  # SharpFuzz rewrites Core.dll to record coverage into a shared memory region that libFuzzer
  # creates. Running that instrumented assembly under a plain `dotnet`, with no fuzzer around it,
  # writes into a region that does not exist:
  #
  #   Fatal error. System.AccessViolationException: Attempted to read or write protected memory.
  #
  # Generating a seed is an ordinary program run, not a fuzz iteration, so it needs the harness and
  # not the instrumentation. **CI had these two the other way round and failed here every night from
  # at least 2026-08-13 to 2026-08-16** — and because a red fuzz workflow is exactly what a real
  # crash looks like, four days of the container target never being fuzzed read as a finding.
  #
  # This script was not failing, only because the seed is skipped once the corpus is non-empty and
  # this box seeded successfully before the ordering mattered. That is luck, not correctness: empty
  # ~/corpus-container and the same crash appears here.
  #
  # The seed is generated from DemoWriter rather than copied from tools/corpus — this box never
  # fetches the demos. The container input opens with an 8-byte magic and a 1072-byte fixed header
  # before anything varies, and mutation from an empty corpus does not cross that reliably.
  if [ -z "$(ls -A "$HOME/corpus-container" 2>/dev/null)" ]; then
    echo "seeding container (before instrumentation)"
    mkdir -p "$HOME/corpus-container"
    TF2FUZZ_SEED_PATH="$HOME/corpus-container/seed" \
      dotnet "${FUZZ_OUT}/Tf2DemoSalvage.Fuzz.dll" 9>&-
  fi
