...
/// the local medic's `_targeted` beam, and the item's custom beam particle.
/// </remarks>
public sealed class HealBeamFeed
{
    ...
    public void Observe(int medigun, bool entering, int? target, bool chargeRelease, int team, int tick)
    {
        if (!entering &&
        ...
        {
            beam = _beams.Count;
            _beams.Add(new SceneHealBeam(medigun, healing, chargeRelease, team, tick, null));
        }
