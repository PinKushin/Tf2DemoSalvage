...
/// `1800fe720`, both read from the disassembly (`docs/findings/51`, *The point-plane evaluator*).
/// </remarks>
    ...
    double InverseApproachSpeed) : IIvpDistanceEvaluator
{
    ...
    /// points are widened from float as they are stored. **The speed is stored with `1.0 / speed`**
    /// (`1800a1c92`–`1800a1cbc`), divided once here and read by the refinement.
    /// </remarks>
    ...
    /// <param name="first">Where the vertex's body is.</param>
    ...
    public double Distance(IvpMatrix first, IvpMatrix second)
    {
        ...
        (double X, double Y, double Z) plane = second.ToWorld(PlanePoint);
