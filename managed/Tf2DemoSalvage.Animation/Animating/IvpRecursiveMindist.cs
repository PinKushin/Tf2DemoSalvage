...
/// <summary>
/// IVP's larger mindist, the 0x100 bytes <c>FUN_1800b21f0</c> builds for a pair either of whose ledges is a hull with children: a plain
/// mindist until it would freeze, or collide on the hull's virtual face, and then the pair's mindists beneath the ledge it opens (B369).
/// </summary>
/// <remarks>
/// **Read from the disassembly** (`docs/findings/51`, *The larger mindist in full*): table <c>1800fe960</c> — slot 0
/// <c>FUN_1800b2250</c>, slot 7 <c>FUN_1800b2700</c>, slot 8 <c>FUN_1800b2460</c> — its own delegator at <c>+0xe0</c> (table
/// <c>1800fe9a8</c>), the mindists beneath it at <c>+0xe8</c>, their total at <c>+0xfc</c>, and the side it opens at <c>+0xf8</c>.
/// **Slot 6, <c>FUN_180028aa0</c>, which answers 1, is not carried**: nothing ported reads it.
/// </remarks>
public sealed class IvpRecursiveMindist : IvpMindist
{
    /// <summary>
    /// <c>DAT_18012d66c</c> — zero in the image, <c>0x3e8</c> once <c>FUN_180002540</c> runs (<c>180002570</c>): past this many mindists
    /// beneath the outermost larger mindist, none opens any further.
    /// </summary>
    ...
    /// <summary><c>DAT_1800fea24</c>, <c>0x58635fa9</c>, <c>1e15f</c>: the radius of a ledge that names no node.</summary>
    ...
    /// <summary>A larger mindist between two features, synapse A being the first — <c>FUN_1800b21f0</c> past the base constructor.</summary>
    ...
    /// <summary>Deletes it — slot 0, <c>FUN_1800b2250</c>: the mindists beneath it (<c>FUN_1800b23a0</c>), then a plain mindist's destructor.</summary>
    ...
    /// <summary>Slot 7, <c>FUN_1800b2700</c>: what a frozen minimize makes of it.</summary>
    ...
    /// <remarks>
    /// <code>
    /// the count (+0xe0's slot 3) &gt; DAT_18012d66c → FUN_180098dd0, FUN_180097ce0 — a plain mindist's invalidation
    /// record 0's ledge without children (+0x8 &amp; 3) → +0xf8 = 1;  else record 1's → 0;  else the radii of their nodes:  rA ≤ rB
    /// FUN_180098dd0;  the range slot 1(A, B);  FUN_180097d60((float)(rA + rB));  flags &amp; 0xffd3ffff | 0x100000;  FUN_1800b29b0(rA + rB)
    /// </code>
    /// </remarks>
    ...
    /// <summary>Slot 8, <c>FUN_1800b2460</c>: the collision event, which opens a ledge when the contact is on a hull's virtual face or edge.</summary>
    ...
    /// <remarks>
    /// <code>
    /// the count &gt; 1000 → FUN_18008ecb0 (a tail call)
    ...
    /// opening:  +0xf8;  FUN_180098dd0(env+0x20);  the range slot 1;  FUN_180097d60;  flags;  FUN_1800b29b0 — slot 7's tail
    /// </code>
    /// </remarks>
    ...
    /// <summary>Told a hull passed a record while open — <c>FUN_1800b28a0</c>, where <c>FUN_180097f00</c> sends the recursive state.</summary>
    /// <param name="recheck">The minimize with no step budget, <c>FUN_180095ad0</c>.</param>
    ...
    /// <remarks>
    /// <code>
    /// FUN_180095ad0;  no bits of 0xc000 and DAT_18012d64c &lt; +0xa8 (COMISS, JNC: a NaN length closes) →
    ///     FUN_1800b23a0, FUN_180098ef0 (both records out of their hull managers), FUN_180097ae0, flags &amp;= ~0x3000
    /// else:  the range slot 1(A, B);  env+0xbc += 1;  FUN_1800b29b0(rA + rB);  record 0 filed again over now with rA, record 1 with rB
    /// </code>
    /// </remarks>
    ...
    private static InvalidOperationException Asserted() =>
        new("A larger mindist's contact pairs feature kinds FUN_1800b2460 asserts against.");
    ...
    /// <summary>
    /// <c>FUN_1800b29b0</c>: the mindists beneath the open side's ledge refreshed — that side queried beneath its ledge as ROOT, the other
    /// handed its ledge as LEDGE — and the change in their number told through the delegator's slot 2.
    /// </summary>
    ...
    /// <summary><c>FUN_1800b23a0</c>: every mindist beneath it deleted, last first, then minus their number told through slot 2.</summary>
    ...
    /// <summary>A larger mindist's delegator, table <c>1800fe9a8</c> at <c>+0xe0</c>.</summary>
    private sealed class Delegation(IvpRecursiveMindist owner) : IIvpCollisionDelegator
    {
        /// <summary>Slot 0, <c>FUN_1800b2320</c>: a mindist beneath it out of the vector by its back-index.</summary>
        public void CollisionRemoved(IvpCollision collision) => IvpCollisionList.Remove(owner._children, collision);

        /// <summary>Slot 2, <c>FUN_1800b2300</c>: <c>+0xfc</c> moved, then the outer delegator's slot 2 (a tail call).</summary>
        ...
        /// <summary>Slot 3, <c>FUN_1800b2860</c>: the outer delegator's count when above zero — asked again, in a tail call — else <c>+0xfc</c>.</summary>
