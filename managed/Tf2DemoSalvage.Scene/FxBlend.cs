...
// **`RenderModes` moved to `Tf2DemoSalvage.Core.Scene`** (B385). `ShouldDraw`'s first test is
// `kRenderNone` and `DemoTimeline` needs it to answer `ShouldInterpolate`, so the constants had to sit
// below Core. What is left here is what a RENDERER does with a mode, which is a different subject.
