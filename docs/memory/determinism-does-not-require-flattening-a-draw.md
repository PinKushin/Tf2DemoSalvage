...
without opening the file, which is [[a-valve-comment-can-be-stale]] applied to our own comments.
...
has not been read yet ([[valve-parity-is-the-first-principle]], [[parity-is-the-first-hypothesis]]).
Where they truly conflict, the seed is the adaptation and the draw is not negotiable
([[an-unrecoverable-input-is-not-an-open-choice]]).

One thing stays flagged rather than claimed: the table's CONTENTS ship only in `particles.lib`, so
ours is uniform and stable but not float-for-float Valve's — [[absent-from-the-sdk-is-not-unreadable]]
names how to settle it.
