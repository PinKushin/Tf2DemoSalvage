...
**An operand-order sabotage that nothing reddens may be dead, not untested.** A NaN's payload is invisible to a comparison, and IVP's heap solve treats a NaN push as no push — so in `IvpFrictionSystem` the destination of every sum feeding only a comparison (the energy test) or a solver result is unobservable at any output. Prove that by tracing each path to an output before hunting for inputs; still write the destination the binary has.
