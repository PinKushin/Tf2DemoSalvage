...
public sealed class ConformanceGapAuditTests
{
    ...
    /// <summary>The suite a marker names, found beside this one.</summary>
    /// <remarks>
    /// **The namespace is READ, not written** (B184). This hardcoded the string
    /// `"Tf2DemoSalvage.Viewer3D.Tests."`, so moving the file to `Rendering.Tests` made the lookup
    /// return null for every suite and the audit reported two markers as deleted that were sitting
    /// next to it.
    ///
    /// **Nothing else could have caught it.** A namespace inside a string is invisible to the
    /// compiler and to the bulk rename that moved 92 files — both saw a string constant. Only the
    /// runtime lookup fails, and only this test reports it, which is why an audit that checks its
    /// own rows still resolve is worth having.
    /// </remarks>
    private static MethodInfo? Method(Marker marker) =>
        typeof(ConformanceGapAuditTests).Assembly
            .GetType($"{typeof(ConformanceGapAuditTests).Namespace}.{marker.Suite}")
