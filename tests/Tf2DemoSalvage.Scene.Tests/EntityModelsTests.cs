...
public sealed class EntityModelsTests
{
    ...
    [Test]
    public void Instances_ABakedPropOnAParent_IsPlacedByTheParent()
    {
        // **`CalcAbsolutePosition`'s third branch, which this loop never ran** (B241). `Simulate`
        // composes the parent chain for a SKINNED model; every BAKED prop was placed at its own
        // pose, and for a parented entity that pose is a LOCAL offset.
        //
        // Every setup gate's grate is a `CDynamicProp` on a `func_door` with a local origin of
        // (0,0,0), so all six drew at the map origin — `[render] door_grate003_top at (-0, 0.1, 0)`
        // in the viewer's own log, for weeks. The doors' brushwork stood where the grates should
        // have been, so nothing looked wrong until `kRenderNone` stopped drawing it.
        EntityModelSet models = new();
        List<ModelInstance> instances = [];

        // **The parent is ROTATED and the child is OFFSET, and both are needed.** A first draft
        // put the child at the parent's own origin with no rotation anywhere, and then
        // `parent ∘ child` and `child ∘ parent` produce the same matrix — the test passed against
        // a deliberately swapped composition order. An input for which correct and broken predict
        // the same observation is not a weak assertion, it is the wrong condition.
        //
        // Parent at (512, 256) turned a quarter turn; child 100 units along its own X. Composed in
        // Valve's order that lands 100 units along the parent's rotated X, which is +Y in world:
        // (512, 356). Swapped, it does not.
        SceneProp[] props =
        [
            Prop("models/props/door.mdl", x: 512f, y: 256f, yaw: 90f, entity: 5),
            Prop("models/props/grate.mdl", x: 100f, entity: 6, attachedTo: 5),
        ];

        models.Add(props, OneTriangle);
        models.Instances(props, instances);

        ModelInstance grate = instances.Single(
            instance => instance.ModelPath.EndsWith("grate.mdl", StringComparison.Ordinal));

        // The translation lives in the last row of the shader's row-vector matrix.
        // **Measured, and it does NOT distinguish the composition order** — swapping
        // `parent.Concat(child)` for `child.Concat(parent)` leaves this at 512 as well. The prop
        // this fixture builds goes down `Simulate`'s skinned path, where `PlacementOf` → `Absolute`
        // returns the PARENT's pose outright, so the branch under test is not the branch that runs.
        //
        // Left in as a placement assertion, which it does make, and marked as NOT a test of the
        // order. The real check is the viewer's own log on a corpus demo: `door_grate003_top at
        // (-0, 0.1, 0)` before, and at the gate after. A fixture that reaches the baked branch is
        // still owed (B241).
        grate.Matrix[12].ShouldBe(512f, 1e-3f);
    }

    [Test]
    public void Instances_ABakedPropWithNoParent_IsPlacedByItsOwnPose()
    {
        // **The control**, and it is the one that stops this becoming "everything follows something".
        // An unparented prop's pose IS its world position, and composing it with anything would
        // move every static prop on the map.
        EntityModelSet models = new();
        List<ModelInstance> instances = [];

        SceneProp[] props = [Prop("models/props/crate.mdl", x: 64f, y: 32f, entity: 7)];

        models.Add(props, OneTriangle);
        models.Instances(props, instances);

        instances.ShouldHaveSingleItem();
        instances[0].Matrix[12].ShouldBe(64f, 1e-4f);
        instances[0].Matrix[13].ShouldBe(32f, 1e-4f);
    }
