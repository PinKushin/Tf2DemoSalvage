...
/// constants' bits, not through this project's code, in metres throughout.
///
...
public sealed class IvpCollisionToleranceConformanceTests
{
    ...
    /// <remarks>
    /// **`block[1]` comes back as `d` itself**, `0x3bcffe5a`, `0.00634745974` metres — `0.9·d + 0.1·d` is `d` again, to the
    /// bit, for this preset.
    /// </remarks>
    [Test]
    public void Margin_InMetres_IsTheToleranceItself() =>
        Bits(IvpCollisionTolerance.Margin).ShouldBe(0x3bcffe5a);

    /// <remarks>
    /// `block[0] = (float)(d · 0.1f)`, `0x3a266515`, `0.000634745988` metres. Subtracted from a pair's distance before the
    /// recheck divides it by the speed bound — `(distance − ε) / speedBound` in `FUN_180099380`.
    /// </remarks>
    [Test]
    public void Epsilon_InMetres_IsATenthOfTheToleranceInFloat() =>
        Bits(IvpCollisionTolerance.Epsilon).ShouldBe(0x3a266515);
    ...
    /// </remarks>
    [Test]
    public void EdgeTargetScale_InMetres_IsATenthOfTheMargin() =>
        Bits(IvpCollisionTolerance.EdgeTargetScale).ShouldBe(0x3a266515);

    /// <remarks>
    /// `block[0x43] = (float)(block[0x42] + d)`, `0x3c4ffe5a`, `0.0126949195` metres — exactly twice the margin. It is
    /// `DAT_18012d64c`, the gap `FUN_180082ed0` starts every contact point with.
    /// </remarks>
    [Test]
    public void ContactGap_InMetres_IsTheRampsEndPlusOneTolerance() =>
    ...
    /// </remarks>
    [Test]
    public void ParallelEdgeGap_InMetres_IsThreeTenthsOfAToleranceBeyondTheContactGap() =>
        Bits(IvpCollisionTolerance.ParallelEdgeGap).ShouldBe(0x3c6f314e);
    ...
    /// `0.579093277` m/s. Below it `FUN_180099380` leaves a pair alone.
    /// </remarks>
    [Test]
    public void ClosingSpeedThreshold_AtSvGravity800_IsTheFallFromTheParallelEdgeGapToTheMargin() =>
        Bits(IvpCollisionTolerance.ClosingSpeedThreshold(PhysicsEnvironment.DefaultGravity * IvpTransform.MetresPerInch))
            .ShouldBe(0x3f143f75);

    /// <remarks>
    /// **A control on the formula's shape, not only its value at one gravity.** The threshold goes as the square root of
    /// `g`, so quadrupling gravity must double it: `0x3f943f75`, `1.15818656` m/s, exactly twice the bits above. A linear or
    /// a constant implementation cannot satisfy both this and the test above.
    /// </remarks>
    [Test]
    public void ClosingSpeedThreshold_AtFourTimesTheGravity_IsDouble() =>
        Bits(IvpCollisionTolerance.ClosingSpeedThreshold(3200f * IvpTransform.MetresPerInch)).ShouldBe(0x3f943f75);
